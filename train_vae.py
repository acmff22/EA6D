...
# !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    ...
    def train(self):
        ...
        # total_batches = len(self.dl.dataset) // self.dl.batch_size
        # epochs_needed = total_steps // total_batches + (total_steps % total_batches != 0)
        # !!!!!!!!!!!
        ...
        with tqdm(initial = self.step, total = self.train_num_steps, disable = not accelerator.is_main_process) as pbar:
            ...
            while self.step < self.train_num_steps:
                ...
                total_loss = 0.
                batch = next(self.dl)
                img = batch['cond'].to(device)
                # !!!!!!!!!!!!!!!!!!!!!!!!!!!!!
                edge = batch['image'].to(device)
                # !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
                # img_normalized = torch.clamp((img + 1.0) / 2.0, 0.0, 1.0)
                # if self.step % 1000 == 0:  # save every 1000 sept
                #     nrow = 2
                #     tv.utils.save_image(img_normalized[:2], f'{self.results_folder}/img_{self.step}.png', nrow=nrow)
                # !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
                ...
                for ga_ind in range(self.gradient_accumulate_every):
                    ...
                    with self.accelerator.autocast():
                        if isinstance(self.model, nn.parallel.DistributedDataParallel):
                            ...
                            loss, log_dict  = self.model.module.training_step(edge, ga_ind, self.step)

                        else:
                            loss, log_dict = self.model.training_step(edge, ga_ind, self.step)
                ...
                if accelerator.is_main_process:
                    ...
                    if self.step != 0 and self.step % self.save_and_sample_every == 0:
                        ...
                        recon_images = torch.clamp((recon_images + 1.0) / 2.0, 0.0, 1.0)
                        original_images = torch.clamp((img[:2] + 1.0) / 2.0, 0.0, 1.0)

                        # Concatenate images along the width to place them side by side
                        combined_images = torch.cat([original_images, recon_images], dim=3)

                        nrow = img[:2].shape[0]  # Number of rows to display in the saved image grid
                        tv.utils.save_image(combined_images,
                                            str(self.results_folder / f'sample-combined-{milestone}.png'), nrow=nrow)

                        # !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
                        # Save individual images for detailed examination
                        tv.utils.save_image(original_images,
                                            str(self.results_folder / f'original_images-{milestone}.jpg'), nrow=nrow)
                        tv.utils.save_image(recon_images, str(self.results_folder / f'recon_images-{milestone}.png'),
                                            nrow=nrow)
                        # !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
                        self.model.train()
                pbar.update(1)
