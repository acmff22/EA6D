...
class LatentDiffusion(DDPM):
    ...
    def get_input(self, batch, return_first_stage_outputs=False, return_original_cond=False):
        ...
        #z=torch.nn.functional.interpolate(x, scale_factor=0.25, mode='bilinear')
    ...
    # entrance of first step
    def training_step(self, batch):
        z, z_img, c, *_ = self.get_input(batch) 
        if self.scale_by_softsign:
            z = F.softsign(z)
        ...
        loss, loss_dict = self(z, z_img, c, edge=_[0])
        return loss, loss_dict
