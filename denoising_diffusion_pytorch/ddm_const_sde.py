...
class DDPM(nn.Module):
    ...
    def forward(self, x, x_img, *args, **kwargs):
        ...
        return self.p_losses(x, x_img, t, *args, **kwargs)
    ...
    def WCE_loss(self, prediction, labelf, beta=1.1):
        ...
        mask[label == 1] = 1.0 * num_negative / (num_posit0ive + num_negative)
...
class LatentDiffusion(DDPM):
    ...
    def rot2quaternion(self,rot):  
        ...
        return torch.from_numpy(quat).to(dtype=torch.float32).cuda()   
    
    @torch.no_grad()
    def get_input(self, batch, return_first_stage_outputs=False, return_original_cond=False):
        assert 'image' in self.input_keys;
       
        rotation=batch['pose'][:, :3, :3]
        que_GT_alloR=self.rot2quaternion(rotation)
        
        x = batch['image']
        cond = batch['cond'] if 'cond' in batch else None 
        z = self.first_stage_model.encode(x)  
        z = self.get_first_stage_encoding(z)  
        z_img = self.first_stage_model.encode(cond)
        z_img = self.get_first_stage_encoding(z_img)
        
        out = [z, z_img, cond, x, que_GT_alloR]
    ...
    def training_step(self, batch):
        ...
        if self.scale_by_softsign:
            ...
            z_img = self.scale_factor * z_img
        x_rec, features, loss, loss_dict = self(z, z_img, c, edge=_[0]) 

        return x_rec, features, loss, loss_dict
    ...
    def quaternion_loss(self, pred_quat, target_rot_matrix):
    ...
    def p_losses(self, x_start, x_img, t, *args, **kwargs):
        if self.start_dist == 'normal':
            noise = torch.randn_like(x_start)
        elif self.start_dist == 'uniform':
            noise = 2 * torch.rand_like(x_start) - 1.
            ...
            raise NotImplementedError(f'{self.start_dist} is not supported !')
        C = x_img-1 * x_start  # U(t) = Ct, U(1) = -x0
        
        #C_gt_pil = T.ToPILImage()(C.squeeze(0))
        #path='/root/DiffusionPose-main-multiStage/test_result/Cgt/'
        #if not os.path.exists(path):
            #os.makedirs(path)
        #C_gt_pil.save(path+str("%06d"%(self.excute_count))+'.png')
        ...
        if self.cfg.model_name == 'cond_unet13':
            ...
            C_pred, noise_pred, features = self.model(x_noisy, t, *args, **kwargs) 
        ...
        simple_weight1 = (t ** 2 + (t - 1) ** 2 + 1) / (t ** 2 + 1)
        simple_weight2 = (t ** 2 + (t - 1) ** 2 + 1) / (1 + (t - 1) ** 2)
        #simple_weight1 = 1
        #simple_weight2 = 1
    ...
    @torch.no_grad()
    def sample(self, batch_size=1, up_scale=1, cond=None, mask=None, denoise=True):
        # image_size, channels = self.image_size, self.channels
        #print("in LatentDiffusion sample")
        ...
        if self.cfg.model_name == 'cond_unet8' or self.cfg.model_name == 'cond_unet13':
            ...
            # 8.13 修改
            z, x0,features = self.sample_fn((batch_size, channels, image_size[0]//down_ratio, image_size[1]//down_ratio),
                               up_scale=up_scale, unnormalize=False, cond=cond, denoise=denoise)
            aux_out = None
        ...
        #x_rec=F.interpolate(z, scale_factor=4, mode='bilinear')
        ...
        return x_rec, x0, features  
        
        # if aux_out is not None:
        #     return img, aux_out, z
        # return img, 
    ...
    def sample_fn(self, shape, up_scale=1, unnormalize=True, cond=None, denoise=False,):
        batch, device, total_timesteps, sampling_timesteps, eta, objective = shape[0], \
            self.betas.device, self.num_timesteps, self.sampling_timesteps, self.ddim_sampling_eta, self.objective
        x_img = self.first_stage_model.encode(cond)  # 
        # print('zzzz', z.shape)
        x_img = self.get_first_stage_encoding(x_img) * self.scale_factor
        # times = torch.linspace(-1, total_timesteps, steps=self.sampling_timesteps + 1).int()
        # times = list(reversed(times.int().tolist()))
        # time_pairs = list(zip(times[:-1], times[1:]))
        # time_steps = torch.tensor([0.25, 0.15, 0.1, 0.1, 0.1, 0.09, 0.075, 0.06, 0.045, 0.03])
        ...
        if self.start_dist == 'normal':
            img = torch.randn(shape, device=device) + x_img
        elif self.start_dist == 'uniform':
            img = 2 * torch.rand(shape, device=device) - 1.
        else:
            raise NotImplementedError(f'{self.start_dist} is not supported !')
        ...
        for i, time_step in enumerate(time_steps):
            ...
            C, noise = pred[:2]
            '''     
            C_pil = T.ToPILImage()(C.squeeze(0))
            path='/root/DiffusionPose-main-multiStage/test_result/C1/'
            if not os.path.exists(path):
                os.makedirs(path)
            C_pil.save(path+str("%06d"%(self.excute_count))+'.png')
            '''
            features=pred[2]
            if self.cfg.model_name == 'cond_unet8' or self.cfg.model_name == 'cond_unet13':
                aux_out = pred[-1]
            ...
            '''
            C_pil = T.ToPILImage()(C.squeeze(0))
            path='/root/DiffusionPose-main-multiStage/test_result/C2/'
            if not os.path.exists(path):
                os.makedirs(path)
            C_pil.save(path+str("%06d"%(self.excute_count))+'.png')
            self.excute_count=self.excute_count+1
            '''
            # img = self.pred_xtms_from_xt(img, noise, C, cur_time, s)
            img = self.q_sample(x0, noise, cur_time - s, C)
