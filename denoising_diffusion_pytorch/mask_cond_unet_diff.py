...
class PositionEmbeddingSine(nn.Module):
    ...
    def forward(self, x):
        ...
        # print(f"Output pos shape: {pos.shape}")
...
#         print('x',x.shape)
#         print('complex_weight',torch.view_as_complex(self.complex_weight).shape)
        
        
#         #  !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
#         # 调整 complex_weight 的形状以匹配 x 的形状
#         complex_weight_adjusted = self.complex_weight
#         if self.complex_weight.shape[2] != x.shape[3]:
#             complex_weight_adjusted = self.complex_weight[:, :, :x.shape[3], :]
#         print(f"x shape before multiplication: {x.shape}")
#         print(f"self.complex_weight shape: {self.complex_weight.shape}")
#         #  !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
class BlockFFT(nn.Module):
    def __init__(self, dim, h, w, groups=8):
        ...
        self.w = w
#        self.complex_weight = nn.Parameter(torch.randn(dim, h, w//2+1, 2, dtype=torch.float32) * 0.02)
        self.complex_weight = nn.Parameter(torch.randn(dim, 8, 5, 2, dtype=torch.float32) * 0.02)
...
#        print('complex_weight', torch.view_as_complex(self.complex_weight).shape)
        ...
        if self.complex_weight.shape[2] != x.shape[3]:
            complex_weight_adjusted = self.complex_weight[:, :, :x.shape[3], :]
        if self.complex_weight.shape[1] != x.shape[2]:
            complex_weight_adjusted = complex_weight_adjusted[:, :x.shape[2], :, :]     
        
#        print('complex_weight', torch.view_as_complex(complex_weight_adjusted).shape)    
        # print(f"x shape before multiplication: {x.shape}")
        # print(f"adjusted complex_weight shape: {complex_weight_adjusted.shape}")

        x = x * torch.view_as_complex(complex_weight_adjusted)
        x = torch.fft.irfft2(x, s=(H, W), dim=(2, 3), norm='ortho')
        x = x.reshape(B, C, H, W)
...
class Unet(nn.Module):
    def __init__(
        ...
        **kwargs
        ...
        #RA-encoder
        self.backbone_feat_dim=3
        self.pose_feat_dim=256
    ...
    def forward(self, x, time, mask, x_self_cond = None, **kwargs):
        ...
        eps = 1e-4
        c_skip1 = (sigma - 1) / (sigma ** 2 + (sigma - 1) ** 2 + 1)
        c_out1 = ((sigma ** 2 + 1) / (sigma ** 2 + (sigma - 1) ** 2 + 1)).sqrt()
        c_skip2 = sigma / (sigma ** 2 + (sigma - 1) ** 2 + 1)
        c_out2 = ((1 + (sigma - 1) ** 2) / (sigma ** 2 + (sigma - 1) ** 2 + 1)).sqrt()
        c_in = 1
        ...
        #print('hm',len(hm),hm[0].shape)
        
        
        # if self.cond_pe:
        #     m = self.cond_pos_embedding(m)
        #print('init_conv_x',torch.cat([x, F.interpolate(hm[0], size=x.shape[-2:], mode="bilinear")], dim=1).shape)
